...
public class MeRequest extends AuthorizedApiRequest {
    ...
    public Builder getUriBuilder() {
        Builder appendQueryParameter = super.getUriBuilder().path(ServerProtocol.USER_ME_PATH).appendQueryParameter("secure_resource", String.valueOf(this.secureResource));
        List<String> list = this.propertyKeyList;
        if (list != null && list.size() > 0) {
            appendQueryParameter.appendQueryParameter(StringSet.propertyKeys, new JSONArray(this.propertyKeyList).toString());
        }
