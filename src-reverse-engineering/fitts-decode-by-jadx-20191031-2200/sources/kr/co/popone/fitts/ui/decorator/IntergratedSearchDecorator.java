...
public final class IntergratedSearchDecorator extends ItemDecoration {
    @NotNull
    private final Context context;
    private Drawable divider = ContextCompat.getDrawable(this.context, this.resId);
    private final int resId;

    public IntergratedSearchDecorator(@NotNull Context context2, int i) {
    ...
    public void getItemOffsets(@NotNull Rect rect, @NotNull View view, @NotNull RecyclerView recyclerView, @NotNull State state) {
        ...
        if (recyclerView.getChildLayoutPosition(view) == 0) {
            rect.top = ViewExtensionsKt.dpToPx(view, 23);
        } else {
            rect.top = ViewExtensionsKt.dpToPx(view, 14);
        }
    }
