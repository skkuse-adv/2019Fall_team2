...
public final class OrderDetailDecorator extends ItemDecoration {
    @NotNull
    private final Context context;
    private Drawable divider = ContextCompat.getDrawable(this.context, this.resId);
    private final int resId;

    public OrderDetailDecorator(@NotNull Context context2, int i) {
    ...
    public void getItemOffsets(@NotNull Rect rect, @NotNull View view, @NotNull RecyclerView recyclerView, @NotNull State state) {
        ...
        if (adapter != null) {
            ...
            if (childLayoutPosition == adapter.getItemCount() - 1) {
                ...
                rect.bottom = ViewExtensionsKt.dpToPx(view, 32);
                return;
            }
            rect.top = ViewExtensionsKt.dpToPx(view, 20);
    ...
    public void onDraw(@NotNull Canvas canvas, @NotNull RecyclerView recyclerView, @NotNull State state) {
        ...
        while (i < childCount) {
            ...
            if (layoutParams != null) {
                int bottom = childAt.getBottom() + ((RecyclerView.LayoutParams) layoutParams).bottomMargin + ViewExtensionsKt.dpToPx(childAt, 18);
